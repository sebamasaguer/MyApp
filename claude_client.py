...
# Contenido de respaldo para cuando no hay API key
_VERSES = [
...
def get_morning_message(pending_from_yesterday: list[str]) -> str:
    if _use_claude():
        try:
            return _morning_claude(pending_from_yesterday)
        except Exception:
            pass
    return _morning_fallback(pending_from_yesterday)


def _morning_claude(pending_from_yesterday: list[str]) -> str:
    pending_section = ""
...
def _morning_fallback(pending_from_yesterday: list[str]) -> str:
...
def get_followup_message(has_tasks: bool) -> str:
    if _use_claude():
        try:
            if has_tasks:
...
def parse_tasks_from_text(user_text: str) -> list[str]:
    if _use_claude():
        try:
...
def get_night_message(tasks: list[dict]) -> str:
    if _use_claude():
        try:
            return _night_claude(tasks)
        except Exception:
            pass
    return _night_fallback(tasks)


def _night_claude(tasks: list[dict]) -> str:
    if not tasks:
        task_section = "No había tareas registradas para hoy."
    else:
        task_section = "\n".join(
...
def _night_fallback(tasks: list[dict]) -> str:
...
def parse_night_review(user_text: str, tasks: list[dict]) -> dict:
    if _use_claude():
        try:
...
Donde done y pending son listas de IDs numéricos de las tareas."""
    ...
    # Fallback: buscar coincidencias por palabras clave en el texto del usuario
...
def get_closing_message(done_count: int, pending_count: int) -> str:
    if _use_claude():
        try:
...
Tono: cálido, espiritual, tranquilizador. Máximo 5 oraciones."""
    ...
    if done_count == 0:
        ...
        balance = f"¡Completaste todo lo que te propusiste! Que orgullo."
    else:
